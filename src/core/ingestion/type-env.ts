...
const resolveFixpointBindings = (
  ...
  for (let iter = 0; iter < MAX_FIXPOINT_ITERATIONS; iter++) {
    ...
    if (!changed) break;
    if (iter === MAX_FIXPOINT_ITERATIONS - 1 && process.env.CODE_DEBUG) {
      ...
      if (typeNode) {
        ...
        if (nameNode) {
          ...
          if (varName && !declarationTypeNodes.has(`${scope}\0${varName}`)) {
            if (typeNode) declarationTypeNodes.set(`${scope}\0${varName}`, typeNode);
          }
        }
      } else {
        ...
        if (fallbackName && fallbackType) {
          ...
          if (varName && !declarationTypeNodes.has(`${scope}\0${varName}`)) {
            if (typeNode) declarationTypeNodes.set(`${scope}\0${varName}`, fallbackType);
          }
        }
      }
