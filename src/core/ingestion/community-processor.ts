...
let leiden: LeidenModule | null = null;
try {
  leiden = _require(leidenPath);
} catch {
  console.warn('[community] Leiden algorithm module not found (vendor/leiden/). Community detection will be skipped.');
}
...
type LeidenModuleOrNull = LeidenModule | null;
...
const generateHeuristicLabel = (
  ...
  const names: string[] = [];
  memberIds.forEach((nodeId) => {
    const name = graph.getNodeAttribute(nodeId, 'name') as string | undefined;
