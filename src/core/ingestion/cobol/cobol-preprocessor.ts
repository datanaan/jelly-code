...
export function extractCobolSymbolsWithRegex(
  ...
  if (execAccum !== null) {
    const ea = execAccum as any;
    if (ea.type === 'sql') {
      result.execSqlBlocks.push(parseExecSqlBlock(ea.lines, ea.startLine));
    } else if (ea.type === 'cics') {
      result.execCicsBlocks.push(parseExecCicsBlock(ea.lines, ea.startLine));
    } else if (ea.type === 'dli') {
      result.execDliBlocks.push(parseExecDliBlock(ea.lines, ea.startLine));
    }
    execAccum = null;
  }
