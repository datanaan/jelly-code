...
/**
 * Load implementor map from Neo4j for incremental mode.
 *
 * In incremental mode (onlyFiles), the in-memory implementorMap only covers
 * the parsed subset of files. Interfaces implemented in unchanged files are
 * missed, causing findInterfaceDispatchTargets to return empty results.
 *
 * This function loads IMPLEMENTS relationships from Neo4j and merges them
 * into the in-memory map, ensuring full coverage.
 *
 * @param store - Neo4j store (externalSymbolStore) for querying
 * @param projectId - Current project ID
 * @param targetMap - In-memory implementor map to augment (mutated in-place)
 * @returns Number of new interface-file entries added
 */
export async function loadImplementorMapFromStore(
  store: { query: (cypher: string, params?: Record<string, unknown>) => Promise<Record<string, unknown>[]> },
  projectId: string,
  targetMap: Map<string, Set<string>>,
): Promise<number> {
  const rows = await store.query(
    `MATCH (child)-[r:CodeRelation {type: 'IMPLEMENTS'}]->(parent)
     WHERE child.projectId = $projectId AND parent.projectId = $projectId
       AND child.filePath IS NOT NULL AND parent.name IS NOT NULL
     RETURN parent.name AS interfaceName, child.filePath AS implFile`,
    { projectId },
  );
  ...
  for (const row of rows) {
    const interfaceName = row.interfaceName as string;
    const implFile = row.implFile as string;
    let files = targetMap.get(interfaceName);
    if (!files) {
      files = new Set();
      targetMap.set(interfaceName, files);
    }
    if (!files.has(implFile)) {
      files.add(implFile);
      added++;
    }
  }
...
// ---------------------------------------------------------------------------
// Route & Tool extraction (sequential path)
// Mirrors the worker's route/tool extraction from parse-worker.ts lines 1299-1370, 1660-1693.
// Uses the same tree-sitter query captures (@decorator, @express_route, @definition).
// ---------------------------------------------------------------------------
...
/**
 * Extract decorator-based routes, Express/Hono routes, and MCP tool definitions
 * from source files using the existing tree-sitter query captures.
 * Designed for the sequential parsing path — mirrors worker logic.
 */
export const extractRoutesAndToolsFromFiles = async (
  files: { path: string; content: string }[],
  astCache: ASTCache,
): Promise<{
  ...
  const decoratorRoutes: ExtractedDecoratorRoute[] = [];
  const toolDefs: ExtractedToolDef[] = [];
  ...
  for (const file of files) {
    ...
    // Track @tool decorators by end-line for definition association
    const fileDecorators = new Map<
      number,
      { name: string; arg: string | undefined; isTool?: boolean }
    >();
    ...
    for (const match of matches) {
      ...
      // --- Decorator-based routes: @Get('/path'), @app.route('/path') ---
      if (captureMap['decorator']) {
        const decoratorNode = captureMap['decorator'];
        const nameNode = captureMap['decorator.name'];
        const argNode = captureMap['decorator.arg'];
        if (nameNode) {
          const decoratorName = nameNode.text;
          const decoratorArg = argNode?.text;
          ...
          // MCP/RPC tool detection: @tool, @mcp.tool(), @server.tool()
      ...
      // --- Tool definition detection: function/class preceded by @tool decorator ---
      // Check for any definition capture (definition.function, definition.class, etc.)
      let defNode: typeof captureMap[string] | undefined;
      for (const defKey of DEFINITION_CAPTURE_KEYS) {
        if (captureMap[defKey]) {
          defNode = captureMap[defKey];
          break;
        }
      }
      if (defNode && captureMap['name']) {
        const MAX_SCAN = 5;
        for (
          let line = defNode.startPosition.row - 1;
          line >= Math.max(0, defNode.startPosition.row - MAX_SCAN);
          line--
        ) {
          const dec = fileDecorators.get(line);
          if (dec?.isTool) {
            // Description: from decorator arg, or fallback to function docstring
            let description = dec.arg || '';
            if (!description) {
              // Extract docstring from function body (Python: expression_statement > string)
              const body = defNode.childForFieldName('body');
              if (body) {
                const firstStmt = body.firstChild;
                if (firstStmt?.type === 'expression_statement') {
                  const strNode = firstStmt.firstChild;
                  if (strNode?.type === 'string') {
                    // Strip triple quotes
                    description = strNode.text.replace(/^['"""]+|['"""]+$/g, '').trim();
                  }
                }
              }
            }
            toolDefs.push({
              filePath: file.path,
              toolName: captureMap['name'].text,
              description,
              lineNumber: defNode.startPosition.row,
            });
            fileDecorators.delete(line);
            break;
          }
        }
      }
  ...
  return { decoratorRoutes, toolDefs };
};
