import type { GraphNode, GraphRelationship, NodeLabel } from '@shared';
...
const processParsingSequential = async (
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    // Load parser for this file's specific language (cached by loadParser)
    // loadParser returns null for unavailable languages — no separate isLanguageAvailable check needed
    ...
    try {
      ...
      console.warn(`Skipping unparseable file: ${file.path} (${language}):`, parseError);
      continue;
    }
