...
import {
  ...
  extractRoutesAndToolsFromFiles,
...
import { type PipelineProgress, getLanguageFromFilename } from '@shared';
 ...
 *
 *  Exported for testing — the implementorMap argument allows injecting
 *  project-wide implementor data in incremental mode. */
...
export interface PipelineOptions {
  ...
  /** Incremental mode: only parse these files (paths relative to repoPath) */
  onlyFiles?: string[];
  /** Neo4j store, for loading exported symbols of files outside onlyFiles */
  externalSymbolStore?: { query: (cypher: string, params?: Record<string, unknown>) => Promise<Record<string, unknown>[]> };
  /** Current projectId */
  projectId?: string;
}
...
async function runScanAndStructure(
  ...
  let scannedFiles = await walkRepositoryPaths(repoPath, (current, total, filePath) => {
  ...
  // Incremental mode: keep allPaths for full discovery, but only parse specified files
  if (options?.onlyFiles && options.onlyFiles.length > 0) {
    const onlyFilesSet = new Set(options.onlyFiles);
    scannedFiles = scannedFiles.filter(f => onlyFilesSet.has(f.path));
    console.log(`[pipeline] onlyFiles mode: ${scannedFiles.length}/${allPaths.length} files selected for parsing`);
  }
...
async function runChunkedParseAndResolve(
  ...
  try {
    for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
      ...
      // Each chunk gets a proportional slice of the 20-82% range.
      // Within each chunk: parsing 50%, imports 15%, heritage 15%, routes 10%, reserve 10%
      const chunkSlice = (62 / numChunks);
      const chunkParsedEnd = chunkBasePercent + chunkSlice * 0.50;
      const chunkImportsEnd = chunkParsedEnd + chunkSlice * 0.15;
      const chunkHeritageEnd = chunkImportsEnd + chunkSlice * 0.15;
      const chunkRoutesEnd = chunkHeritageEnd + chunkSlice * 0.10;
      ...
      if (chunkWorkerData) {
        ...
        await processImportsFromExtracted(
          ...
          ctx,
          (current, total) => {
            const importFrac = total > 0 ? current / total : 1;
            onProgress({
              phase: 'parsing',
              percent: Math.round(chunkParsedEnd + importFrac * (chunkImportsEnd - chunkParsedEnd)),
        ...
        await Promise.all([
          processHeritageFromExtracted(graph, chunkWorkerData.heritage, ctx, (current, total) => {
            const heritageFrac = total > 0 ? current / total : 1;
            onProgress({
              phase: 'parsing',
              percent: Math.round(chunkImportsEnd + heritageFrac * (chunkHeritageEnd - chunkImportsEnd)),
          ...
          processRoutesFromExtracted(graph, chunkWorkerData.routes ?? [], ctx, (current, total) => {
            const routeFrac = total > 0 ? current / total : 1;
            onProgress({
              phase: 'parsing',
              percent: Math.round(chunkHeritageEnd + routeFrac * (chunkRoutesEnd - chunkHeritageEnd)),
    ...
    // P2: In incremental mode, augment implementorMap with Neo4j data so
    // interface dispatch finds implementors from non-incremental files.
    if (externalSymbolStore && projectId && deferredWorkerCalls.length > 0) {
      const added = await loadImplementorMapFromStore(externalSymbolStore, projectId, fullWorkerImplementorMap);
      if (added > 0) {
        const scannedPaths = new Set(scannedFiles.map(f => f.path));
        let outsideCount = 0;
        for (const [, implFiles] of fullWorkerImplementorMap) {
          for (const f of implFiles) {
            if (!scannedPaths.has(f)) outsideCount++;
          }
        }
        if (isDev) {
          console.log(`[pipeline] Loaded ${added} implementor entries from Neo4j (${outsideCount} outside onlyFiles)`);
        }
      }
    }
    ...
    if (deferredWorkerCalls.length > 0) {
      await processCallsFromExtracted(
        ...
        ctx,
        (current, total) => {
          const callFrac = total > 0 ? current / total : 1;
          onProgress({
            phase: 'parsing',
            percent: Math.round(82 + callFrac * 5), // 82% → 87% for call resolution
  ...
  // P2: In incremental mode, seed sequential implementorMap from Neo4j
  if (externalSymbolStore && projectId) {
    const added = await loadImplementorMapFromStore(externalSymbolStore, projectId, sequentialImplementorMap);
    if (added > 0 && isDev) console.log(`[pipeline] Sequential: loaded ${added} implementor entries from Neo4j`);
  }
  for (const chunkPaths of sequentialChunkPaths) {
    ...
    // Extract routes and MCP tools (sequential path)
    const chunkRoutesTools = await extractRoutesAndToolsFromFiles(chunkFiles, astCache);
    if (chunkRoutesTools.decoratorRoutes.length > 0) {
      allDecoratorRoutes.push(...chunkRoutesTools.decoratorRoutes);
    }
    if (chunkRoutesTools.toolDefs.length > 0) {
      allToolDefs.push(...chunkRoutesTools.toolDefs);
    }
...
/**
 * Load exported symbols from Neo4j for files outside the incremental set.
 *
 * In incremental mode with onlyFiles, only a subset of files are parsed.
 * Other files' exported symbols must be loaded from Neo4j to support
 * import resolution. If a symbol is not found, checkImportsResolvable
 * will trigger an IncrementalFallbackError.
 */
async function loadExternalSymbols(
  ctx: ReturnType<typeof createResolutionContext>,
  store: NonNullable<PipelineOptions['externalSymbolStore']>,
  projectId: string,
  allPaths: string[],
  onlyFiles: string[],
): Promise<void> {
  const externalFiles = allPaths.filter(p => !onlyFiles.includes(p));
  if (externalFiles.length === 0) return;

  const BATCH_SIZE = 1000;
  let totalLoaded = 0;

  for (let i = 0; i < externalFiles.length; i += BATCH_SIZE) {
    const batch = externalFiles.slice(i, i + BATCH_SIZE);
    const result = await store.query(
      `MATCH (n) WHERE n.projectId = $projectId AND n.filePath IN $files
       AND (n:Function OR n:Class OR n:Interface OR n:Method
            OR n:Constructor OR n:Property OR n:CodeElement)
       RETURN n.filePath AS filePath, n.name AS name, n.id AS id, n.type AS type,
              n.parameterCount AS parameterCount,
              n.requiredParameterCount AS requiredParameterCount,
              n.parameterTypes AS parameterTypes, n.returnType AS returnType,
              n.declaredType AS declaredType, n.ownerId AS ownerId`,
      { projectId, files: batch },
    );
    ...
    for (const row of result) {
      ctx.symbols.add(
        ...
        row.name as string,
        row.id as string,
        row.type as string as any, // type is NodeLabel
        {
          parameterCount: row.parameterCount as number | undefined,
          requiredParameterCount: row.requiredParameterCount as number | undefined,
          parameterTypes: row.parameterTypes as string[] | undefined,
          returnType: row.returnType as string | undefined,
          declaredType: row.declaredType as string | undefined,
  ...
  console.log(`[pipeline] Loaded ${totalLoaded} external symbols from Neo4j for ${externalFiles.length} files`);
}

/**
 * Check that all named imports from onlyFiles files are resolvable.
 *
 * If an import's source file is NOT in onlyFiles, and the target symbol
 * cannot be found in ctx.symbols (which includes both freshly-parsed
 * symbols and Neo4j-loaded external symbols), throw IncrementalFallbackError.
 *
 * This implements principle 4: import 解析失败 = 触发全量.
 */
function checkImportsResolvable(
  ctx: ReturnType<typeof createResolutionContext>,
  onlyFiles: string[],
): void {
  if (!onlyFiles || onlyFiles.length === 0) return;

  const onlyFilesSet = new Set(onlyFiles);

  for (const [callerFile, imports] of ctx.namedImportMap.entries()) {
    // Only check imports from files in the incremental set
    if (!onlyFilesSet.has(callerFile)) continue;

    for (const [, binding] of imports.entries()) {
      // If source file is also in onlyFiles, it will be parsed — skip
      if (onlyFilesSet.has(binding.sourcePath)) continue;

      // Look up target symbol in ctx.symbols
      const candidates = ctx.symbols.lookupFuzzy(binding.exportedName);
      const fromSource = candidates.filter(c => c.filePath === binding.sourcePath);

      if (fromSource.length === 0) {
        throw new IncrementalFallbackError(
          `Cannot resolve import "${binding.exportedName}" from "${callerFile}" ` +
          `(source: ${binding.sourcePath}). The symbol is not in onlyFiles and not in Neo4j.`,
...
export const runPipelineFromRepo = async (
  ...
  try {
    ...
    // Incremental mode: load external symbols from Neo4j BEFORE parsing
    // so that CALLS / EXTENDS / IMPLEMENTS edge resolution can find them.
    if (options?.onlyFiles && options.externalSymbolStore && options.projectId) {
      await loadExternalSymbols(ctx, options.externalSymbolStore, options.projectId, allPaths, options.onlyFiles);
    }
    ...
    const {
      ...
      pipelineStart,
      onProgress,
      options?.externalSymbolStore,
      options?.projectId,
    );

    // Verify resolvability AFTER parsing — as a safety net for any missed symbols
    if (options?.onlyFiles && options.externalSymbolStore && options.projectId) {
      checkImportsResolvable(ctx, options.onlyFiles);
    }

    // ── Phase 3.5: Route Registry (Next.js + PHP + Laravel + decorators) ──
    type RouteEntry = { filePath: string; source: string; method?: string };
    ...
    const addRoute = (url: string, entry: RouteEntry) => {
      const key = entry.method ? `${entry.method}:${url}` : url;
      if (routeRegistry.has(key)) {
        duplicateRoutes++;
        // Route conflict detection (Task 4.4): log warning if different files define the same route
        const existing = routeRegistry.get(key)!;
        if (entry.filePath !== existing.filePath) {
          console.warn(`[pipeline] Route conflict: ${key} defined in both ${existing.filePath} and ${entry.filePath}`);
        }
        return;
      }
    ...
    for (const dr of allDecoratorRoutes) {
      addRoute(ensureSlash(dr.routePath), {
        ...
        method: dr.httpMethod || undefined,
      });
    }

    let handlerContents: Map<string, string> | undefined;
    if (routeRegistry.size > 0) {
      let handlerPaths = [...routeRegistry.values()].map((e) => e.filePath);
      // Incremental mode: only read content for files in the incremental set
      // (others' route data already exists in Neo4j and will be preserved)
      if (options?.onlyFiles) {
        const onlyFilesSet = new Set(options.onlyFiles);
        handlerPaths = handlerPaths.filter(p => onlyFilesSet.has(p));
      }
      ...
      for (const [routeKey, entry] of routeRegistry) {
        ...
        // Extract clean URL from composite key (METHOD:URL → URL, or plain URL)
        const routeURL = routeMethod ? routeKey.slice(routeMethod.length + 1) : routeKey;
        const routeNodeId = generateId('Route', routeURL);
        graph.addNode({
          id: routeNodeId,
          label: 'Route',
    ...
    if (toolCandidatePaths.length > 0) {
      // Incremental mode: only read content for files in the incremental set
      // (tools in unchanged files already exist in Neo4j)
      let toolReadPaths = toolCandidatePaths;
      if (options?.onlyFiles) {
        const onlyFilesSet = new Set(options.onlyFiles);
        toolReadPaths = toolCandidatePaths.filter(p => onlyFilesSet.has(p));
        if (toolReadPaths.length !== toolCandidatePaths.length) {
          console.log(`[pipeline] Tool detection: scanning ${toolReadPaths.length}/${toolCandidatePaths.length} files (incremental mode)`);
        }
      }
      const toolContents = await readFileContents(repoPath, toolReadPaths);
    ...
    return { graph, repoPath, totalFileCount: totalFiles, communityResult, processResult } as any;
  } catch (error) {
    ctx.clear();
    throw error;
  }
